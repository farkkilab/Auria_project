...
umap_df <- as.data.frame(Embeddings(TNBC, reduction = "UMAP"))
umap_df$signature <- TNBC$signature_GCLCVIM
colnames(umap_df) <- c("UMAP_1", "UMAP_2", "signature")
max(TNBC$signature_GCLCVIM)
min(TNBC$signature_GCLCVIM)



ggplot(umap_df, aes(x=UMAP_1, y= UMAP_2, color = signature)) + 
  geom_point(size=0.3, alpha=0.33) +
  scale_color_gradientn(colors = c( "#67A9CF", "#D1E5F0", "#FDAE61", "#D73027"), limits = c(0, 0.3)) +
  theme_classic() + 
  theme(legend.position = "right", legend.key.size = unit(0.5, "cm"),panel.background = element_blank(),
        axis.line = element_blank(),       # Remove axis lines
        axis.ticks = element_blank(),      # Remove axis ticks
        axis.text = element_blank(),       # Remove axis text
        axis.title = element_blank()) +
  labs(color = 'GCLC_VIM_signatures')
...
# UMAP of GCLC_VIM in only tumor cells (Cancer Epithelial) for 7 TNBC patients

# Subset to tumor cells
TNBC_tumor <- subset(TNBC, celltype_major == "Cancer Epithelial")

# Check available patients
unique(TNBC_tumor$Patient)

# If you want to select 7 specific patients, specify them here:
selected_patients <- c("CID4495", "CID44971", "CID44991", "CID4513", "CID4515", "CID4523", "CID3963")
TNBC_tumor_7 <- subset(TNBC_tumor, Patient %in% selected_patients)

# Extract UMAP and signature
umap_tumor_df <- as.data.frame(Embeddings(TNBC_tumor_7, reduction = "UMAP"))
umap_tumor_df$signature <- TNBC_tumor_7$signature_GCLCVIM
umap_tumor_df$Patient <- TNBC_tumor_7$Patient

colnames(umap_tumor_df)[1:2] <- c("UMAP_1", "UMAP_2")

# Plot
ggplot(umap_tumor_df, aes(x = UMAP_1, y = UMAP_2, color = signature)) +
  ...
  labs(color = 'GCLC_VIM_signatures', title = "UMAP of Tumor Cells (7 TNBC Patients)")

ggsave("/Users/wenqchen/Desktop/Projects/Auria/Plots/scRNA/Oxstress/GCLC_VIM_TNBC_umap_tumor_7patients.pdf",
